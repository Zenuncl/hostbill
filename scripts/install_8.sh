...
# CentOS Linux/Stream 8, Rocky 8, AlmaLinux 8.x +  php-7.4
# Rev: 2023-02-23
...
check_os()
{
	...
	grep "CentOS Linux release 8." /etc/redhat-release -i -q;
	OS=`echo $?`;
	grep "Rocky Linux release 8." /etc/redhat-release -i -q;
	ROCK=`echo $?`;
	grep "CentOS Stream release 8"  /etc/redhat-release -i -q;
	STREAM=`echo $?`;
	grep "AlmaLinux release 8."  /etc/redhat-release -i -q;
	ALMA=`echo $?`;
	...
	if [ $OS -ne 0 ] && [ $ROCK -ne 0 ] && [ $STREAM -ne 0 ]  && [ $ALMA -ne 0 ]; then
		echo "This install script requires Centos Linux or Stream 8.x or Rocky Linux 8.x or AlmaLinux 8.x"
		exit 1
	fi
	if [ $ARCH != "x86_64" ]; then
...
dnf -y install https://dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm >> $LOG 2>&1
dnf -y install https://rpms.remirepo.net/enterprise/remi-release-8.rpm >> $LOG 2>&1
...
dnf -y module install php:remi-7.4  >> $LOG 2>&1
